...
4. **Record rationale** - after each resolution, create a rationale entry (id prefix `RAT-NNN`, template in `assets/templates/rat-entry.md`) stating: the conflict, the chosen option, why, and `links` with kind `clarifies` pointing at every entry it affected. Rebuild the index afterwards.
5. **Resume the pipeline** - return to the main workflow in `../SKILL.md` at the detect/gate step. Never skip re-detection after resolutions.
